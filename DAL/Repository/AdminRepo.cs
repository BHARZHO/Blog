...
public class AdminRepo : IAdmin
    {
        ...
        {
            var currentUser = await BlogApplication.Utility.Helper.GetCurrentUserIdAsync(_httpContextAccessor, _userManager);
        ...
        {
            var rowAffected = await _appDb.Blogs.Where(x => x.ID == editEntity.ID).ExecuteUpdateAsync(s => s.SetProperty(e => e.Name, editEntity.Name).SetProperty(e => e.Body, editEntity.Body));
            return rowAffected != 0;
